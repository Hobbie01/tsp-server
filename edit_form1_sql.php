...
$strSQL = " update `table1` SET `a1` = '$a1', `pname` = '$pname', `fname` = '$fname', `lname` = '$lname', `age` = '$age', `HN` = '$HN', `AN` = '$AN', `DX` = '$DX', `UD` = '$UD', `date_in` = '$date_in', `date_out` = '$date_out', `date_move` = '$date_move', `from_ward` = '$from_ward', `date_infect` = '$date_infect', `ward_infect` = '$ward_infect', `dep_infect` = '$dep_infect', `status1` = '$status1', `status2` = '$status2', `st` = '$st', `off` = '$off', `other_in` = '$other_in', `cauti` = '$cauti', `non_cauti` = '$non_cauti', `timestamp` = '$timestamp' WHERE `table1_id` = '$table1_id'";
$objQuery = mysqli_query($objCon,$strSQL);

$a = " select * from `table1` WHERE table1_id = '".$_POST["table1_id"]."' Order By table1_id DESC Limit 1";
$aQuery = mysqli_query($objCon,$a);
while($row = mysqli_fetch_array($aQuery)){
$row["table1_id"];
    if($row["table1_id"]!=""){
        echo '<meta http-equiv="refresh" content="0; url=edit_form3.php?table1_id='.$row["table1_id"].'">'; 
