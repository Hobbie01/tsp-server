...
$config = [
    'a' => [
        ...
        'columns' => ['HN', 'AN', 'name'],
        'has_additional_forms' => true,
        'additional_forms' => [
            ['number' => 2, 'title' => 'ทบทวนปัจจัย', 'icon' => 'fa-clipboard-check', 'description' => 'แบบฟอร์มทบทวนปัจจัยการติดเชื้อในระบบทางเดินปัสสาวะที่สัมพันธ์ในการใส่สายสวนปัสสาว(CAUTI)'],
            ['number' => 3, 'title' => 'เพิ่มการวินิจฉัย', 'icon' => 'fa-plus-circle', 'description' => 'เพิ่มการวินิจฉัยการติดเชื้อในระบบทางเดินปัสสาวะ(ต่อ)'],
            ['number' => 4, 'title' => 'ผลการตรวจ', 'icon' => 'fa-microscope', 'description' => 'การวินิจฉัยการติดเชื้อในระบบทางเดินปัสสาวะ(ต่อ)']
        ]
    ],
    'b' => [
...
// เพิ่มฟังก์ชันสำหรับการนำทางไปยังฟอร์มต่างๆ (สำหรับประเภท a เท่านั้น)
                        ...
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle text-end">
                                        <?php if ($current_config['has_additional_forms']): ?>
                                            <?php foreach ($current_config['additional_forms'] as $form): ?>
                                                <button onclick="goToForm(<?= $form['number'] ?>, '<?= $row[$current_config['id_field']] ?>', '<?= $row["HN"] ?>', '<?= $row["AN"] ?>')"
                                                    class="btn btn-outline-success px-3 py-2 me-2" title="<?= $form['description'] ?>">
                                                    <i class="fa <?= $form['icon'] ?>"></i> <?= $form['title'] ?>
                                                </button>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
